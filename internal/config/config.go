...
func ResolveProvider(providerFlag string) (provider.Provider, error) {
	p := providerFlag
	if p == "" {
	...
	case "openai":
		...
		return openai.New(openaiKey, os.Getenv("OPENAI_BASE_URL")), nil
	case "anthropic":
		if anthropicKey == "" {
			return nil, fmt.Errorf("ANTHROPIC_API_KEY not set")
		}
		return anthropic.New(anthropicKey, os.Getenv("ANTHROPIC_BASE_URL")), nil
	case "":
		return autoDetect(openaiKey, anthropicKey)
	default:
		return nil, fmt.Errorf("unknown provider %q (use 'openai' or 'anthropic')", p)
	}
}

func autoDetect(openaiKey, anthropicKey string) (provider.Provider, error) {
	hasOpenAI := openaiKey != ""
